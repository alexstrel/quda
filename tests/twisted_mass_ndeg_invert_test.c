...
int main(int argc, char **argv)
{
  ...
  void *flavorSpinorIn[2];
  void *flavorSpinorOut[2];
  void *flavorSpinorCheck[2];
  
  for(int f = 0; f < 2; f++)
  {
      flavorSpinorIn[f] = malloc(V*spinorSiteSize*sSize);
      flavorSpinorOut[f] = malloc(V*spinorSiteSize*sSize);
      flavorSpinorCheck[f] = malloc(V*spinorSiteSize*sSize);
  }
  ...
  construct_spinor_field(flavorSpinorIn[0], 1, 0, 0, 0, inv_param.cpu_prec);
  
  construct_spinor_field(flavorSpinorIn[1], 1, 0, 0, 0, inv_param.cpu_prec);
  ...
  printf("\nStart inversion: \n");
  invertMultiFlavorQuda((void*)flavorSpinorOut[0], (void*)flavorSpinorOut[1], (void*)flavorSpinorIn[0], (void*)flavorSpinorIn[1], &inv_param);
  ...
  if (inv_param.solution_type == QUDA_MAT_SOLUTION) { 
    ndeg_mat(flavorSpinorCheck[0], flavorSpinorCheck[1], gauge, flavorSpinorOut[0], flavorSpinorOut[1], inv_param.kappa, inv_param.mu, inv_param.epsilon, 
  ...
  for(int f = 0; f < 2; f++)
  {
	mxpy(flavorSpinorIn[f], flavorSpinorCheck[f], V*spinorSiteSize, inv_param.cpu_prec);
	double nrm2 = norm_2(flavorSpinorCheck[f], V*spinorSiteSize, inv_param.cpu_prec);
	double src2 = norm_2(flavorSpinorIn[f], V*spinorSiteSize, inv_param.cpu_prec);
